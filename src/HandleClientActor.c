...
void* handle_client_actor(void *arg){
	...
	BlockingQueue* Q = ((struct client_actor_package*)arg)->handle_client_actor_q;
	BlockingQueue* LOG = ((struct client_actor_package*)arg)->log_actor_q;
	...
	while(1){
		...
		conn_ptr = BlockingQueue_remove(Q);
		...
		log_message = malloc(strlen(log_buffer));
		if(log_message){
			memcpy(log_message, log_buffer, strlen(log_buffer));
			BlockingQueue_add(LOG, log_message);
		}
